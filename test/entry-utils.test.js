...
import { buildEntryLines, formatTimestamp, prepareEntry } from '../src/tui/entry-utils.js';
...
describe('buildEntryLines', () => {
  test('compact command without output', () => {
    const lines = buildEntryLines(sampleCommand, Infinity, 80, { showOutput: false, compact: true });
  ...
  test('full command with output', () => {
    const lines = buildEntryLines(sampleCommand, Infinity, 80, { showOutput: true });
  ...
  test('truncates long commands in compact mode', () => {
    ...
    const lines = buildEntryLines(longCommand, Infinity, 80, { showOutput: false, compact: true });
    assert.strictEqual(lines[1].type, 'command');
  ...
  test('shows full command in detail view', () => {
    ...
    const lines = buildEntryLines(longCommand, Infinity, 80, { showOutput: true, compact: false });
    assert.strictEqual(lines[1].type, 'command');
  ...
  test('handles multi-line commands', () => {
    ...
    const lines = buildEntryLines(multilineCommand, Infinity, 80, { showOutput: false, compact: true });
    assert.strictEqual(lines[1].type, 'command');
