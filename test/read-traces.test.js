...
import os from 'os';
import { readTraceEntries, listSessions, getTraceFile } from '../src/tui/read-traces.js';
...
describe('read-traces helpers', () => {
  ...
  test('listSessions and readTraceEntries', async () => {
    const projDir = path.join(tmpHome, '.dockashell', 'projects', 'proj', 'traces', 'sessions');
    ...
    await fs.utimes(ses1, new Date(Date.now() - 2000), new Date(Date.now() - 2000));
    await fs.utimes(ses2, new Date(Date.now() - 1000), new Date(Date.now() - 1000));
